ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS local_number text DEFAULT '';
...
BEGIN
  ...
  INSERT INTO public.profiles (id, email, name, store_name, store_floor, store_id, local_number, is_employee, phone, promo_code, account_type)
  VALUES (
    NEW.id,
    NEW.email,
    COALESCE(meta->>'name', ''),
  ...
  ON CONFLICT (id) DO UPDATE SET
    ...
    local_number = COALESCE(NULLIF(EXCLUDED.local_number,''), public.profiles.local_number);
